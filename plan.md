...
- 115 deterministic tests spanning direct-only goals, real public Pi AgentSession completion, custom-turn dispatch and dropped-delivery faults, FIFO tool serialization, busy cancellation, namespace recovery, foreign-turn isolation, optional owned work, state overflow persistence, bridge terminal/start/update normalization and index-only incremental array-copy safety, runner resource bounds/deadlines, hostile review payload rendering, status API, and extension behavior;
